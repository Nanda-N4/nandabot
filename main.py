...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Business message ရော ရိုးရိုး message ရော လက်ခံမယ်
...
async def handle_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin ကိုယ်တိုင် Chat ထဲစာရိုက်တာကို Echo မဖြစ်အောင် (Business မဟုတ်ရင် ကျော်မယ်)
    ...
    if msg.photo:
        ...
        # Admin ဆီကို Button နဲ့တကွ ပို့မယ်
...
async def handle_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Approve Logic (ဒီနေရာမှာ user ဆီ စာရောက်အောင် သေချာပို့မယ်)
    if data.startswith('ap_'):
        ...
        if db.update_balance(target_id, amt, "TOPUP_APPROVE"):
            # User ဆီကို Bot ကနေ တိုက်ရိုက်စာပို့ခိုင်းမယ်
            try:
                ...
                logging.error(f"Error sending noti to user: {e}")
        ...
        # Start menu ကို callback ကနေ ခေါ်တဲ့အခါ query.message ကို သုံးရတယ်
        ...
        # Navigation logic အပြည့်အစုံ
        if data == 'topup_menu':
            kb_topup = [[InlineKeyboardButton("💳 5,000 Ks", callback_data='pay_5000')],
            ...
            await query.edit_message_text("ဖြည့်သွင်းလိုသော ပမာဏ ရွေးချယ်ပါ-", reply_markup=InlineKeyboardMarkup(kb_topup))
        elif data == 'my_acc':
            await query.edit_message_text(f"👤 **Account Info**\n🆔 ID: `{uid}`\n💰 Balance: **{bal} Ks**", 
                                          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📜 History", callback_data='view_history')], [InlineKeyboardButton("🔙 Back", callback_data='back_to_main')]]), 
                                          parse_mode='Markdown')
        else:
        ...
        txt = "📜 **မှတ်တမ်း (၅) ခု**\n\n"
        for amt, t, ts in h:
            txt += f"{'➕' if amt > 0 else '➖'} {abs(amt)} Ks ({t})\n"
        ...
        # (XUI logic ကျန်သေးရင် ထည့်ရန်)
        await query.edit_message_text(f"✅ **{p['name']}** အောင်မြင်ပါသည်။")
        await backup_to_admin(context)
...
async def admin_credit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if db.update_balance(uid, amt, "ADMIN_ADD"):
            await context.bot.send_message(uid, f"✅ Admin မှ Credit **{amt} Ks** ဖြည့်သွင်းပေးလိုက်ပါသည်။", parse_mode='Markdown')
